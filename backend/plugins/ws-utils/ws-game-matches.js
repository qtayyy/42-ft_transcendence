		...
		if (tournament) {
			...
			if (updateResult.success) {
				...
				if (tournament.isComplete()) {
					console.log(`Tournament ${gameState.tournamentId} is complete. Cleaning up room and tournament.`);
					...
					if (room) {
						console.log(`Deleting room ${roomId} for completed tournament ${gameState.tournamentId}`);
						...
						console.log(`Room ${roomId} not found for tournament ${gameState.tournamentId} (may have been cleaned up already)`);
					}
					...
					setTimeout(() => {
						...
						console.log(`Removed completed tournament ${gameState.tournamentId} from activeTournaments`);
	...
	fastify.decorate("handlePlayerNavigatingAway", (matchId, userId) => {
		...
		if (!gameState[timeoutKey]) {
			gameState[timeoutKey] = setTimeout(() => {
				...
				const disconnectedId = disconnectedPlayer === "LEFT" ? gameState.leftPlayer?.id : gameState.rightPlayer?.id;
				if (disconnectedId) {
					console.log(`[Navigate Away] Evicting disconnected user ${disconnectedId} from room ${gameState.roomId}`);
