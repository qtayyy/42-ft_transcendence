...
function broadcastState(gameState, fastify) {
  ...
  if (gameState.paused && gameState.disconnectedPlayer && gameState.pausedAt) {
    ...
    disconnectCountdown = {
      ...
      countdown: Math.max(
        0,
        Math.ceil((gracePeriodEndsAt - Date.now()) / 1000),
      ),
    };
  }
...
export default fp(async (fastify, opts) => {
  ...
  fastify.decorate("handlePlayerNavigatingAway", (matchId, userId) => {
    ...
    if (gameState.isTournamentMatch && gameState.tournamentId) {
      console.log(
        `[Navigate Away] Processing tournament forfeit for ${matchId}`,
      );
      if (fastify.handleTournamentMatchEnd) {
        ...
        const winnerId =
          disconnectedPlayer === "LEFT"
            ? gameState.rightPlayer.id
            : gameState.leftPlayer.id;
        fastify.handleTournamentMatchEnd(
    ...
    if (!gameState[timeoutKey]) {
      gameState[timeoutKey] = setTimeout(() => {
        ...
        if (bothDisconnected) {
          console.log(
            `[Navigate Away] Both players forfeited - double forfeit`,
          );
          gameState.gameOver = true;
  ...
  fastify.decorate(
    "startRematch",
    (player1Id, player1Username, player2Id, player2Username) => {
  ...
  fastify.decorate(
    ...
    (matchId, tournamentId, player1Id, player1Name, player2Id, player2Name) => {
  ...
  fastify.decorate("getGameState", (matchId, userId) => {
    ...
    const isSpectator = fastify.matchSpectators
      ?.get(matchId)
      ?.has(Number(userId));
