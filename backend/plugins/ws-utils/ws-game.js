...
export default fp((fastify) => {
	...
	fastify.decorate("joinMatchmaking", (userId, username, mode) => {
		...
		if (staleRoomId) {
			...
			if (room && room.isMatchmade && !room.tournamentStarted) {
				...
				console.log(`[Matchmaking] Silently clearing User ${numericUserId} from stale room ${staleRoomId}`);
	...
	fastify.decorate("leaveMatchmaking", (userId) => {
		...
		matchmakingQueue.single = matchmakingQueue.single.filter((p) => p.userId !== numericUserId);
		matchmakingQueue.tournament = matchmakingQueue.tournament.filter((p) => p.userId !== numericUserId);
		...
		if (currentRoomId) {
			...
			if (room) {
				...
				if (room.joinedPlayers.length === 0 && room.isMatchmade) {
					...
					console.log(`[Matchmaking] Deleted empty matchmade room ${currentRoomId}`);
			...
			fastify.currentRoom.delete(numericUserId);
			console.log(`[Matchmaking] User ${numericUserId} left room ${currentRoomId}`);
		}
