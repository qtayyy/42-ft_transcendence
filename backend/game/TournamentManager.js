...
class TournamentManager {
  ...
  updateStandings(matchResult) {
    ...
    const p2Standing = player2Id
      ? this.standings.find((s) => s.playerId === player2Id)
      : null;
    ...
    if (outcome === "double_forfeit") {
      ...
      const p1Standing = this.standings.find(
        (s) => Number(s.playerId) === Number(player1Id),
      );
      const p2Standing = player2Id
        ? this.standings.find((s) => Number(s.playerId) === Number(player2Id))
        : null;
    ...
    if (outcome === "forfeit" || outcome === "walkover") {
      ...
      if (!winnerId) {
        ...
        console.warn(
          `Tournament ${this.tournamentId}: Forfeit/walkover without explicit winnerId. Falling back to score.`,
        );
      ...
      const loserId =
        Number(winnerId) === Number(player1Id)
          ? Number(player2Id)
          : Number(player1Id);

      const winnerStanding = this.standings.find(
        (s) => Number(s.playerId) === winnerId,
      );
      const loserStanding = this.standings.find(
        (s) => Number(s.playerId) === loserId,
      );
      ...
      if (winnerStanding) {
        ...
        if (outcome === "forfeit") {
          winnerStanding.totalPointsScored +=
            score.p1 > score.p2 ? score.p1 : score.p2;
      ...
      if (loserStanding) {
        ...
        // For walkover, no score differential change (0-0 score)
        if (outcome === "forfeit") {
          loserStanding.totalPointsScored +=
            score.p1 > score.p2 ? score.p2 : score.p1;
  ...
  resolveFutureMatchesForWithdrawn(withdrawnPlayerId) {
    ...
    this.matches.forEach((match) => {
      ...
      const isP1Withdrawn =
        match.player1 && Number(match.player1.id) === Number(withdrawnPlayerId);
      const isP2Withdrawn =
        match.player2 && Number(match.player2.id) === Number(withdrawnPlayerId);
  ...
  checkAndProcessRoundBye(round) {
    ...
    const allRealMatchesComplete = realMatches.every(
    ...
    if (allRealMatchesComplete) {
      ...
      const byeMatch = roundMatches.find(
        (m) => m.player2 === null && m.status === "pending",
      );
      ...
      // Otherwise generate next round pairings (Swiss only)
        ...
        if (existingMatches.length > 0) {
          ...
          try {
            ...
            console.error(
              `Tournament ${this.tournamentId}: Failed to generate Swiss pairings:`,
              err,
            );
          }
        }
    ...
    return [...this.standings]
      .map((s) => ({
        ...
        avgScoreDifferential:
          s.matchesPlayed > 0
            ? Number((s.scoreDifferential / s.matchesPlayed).toFixed(2))
            : 0,
        avgTotalPointsScored:
          s.matchesPlayed > 0
            ? Number((s.totalPointsScored / s.matchesPlayed).toFixed(2))
            : 0,
      }))
      .sort((a, b) => {
