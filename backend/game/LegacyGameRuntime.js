...
const AI_PROFILES = {
  easy: {
    decisionIntervalMinMs: 200,
    decisionIntervalMaxMs: 320,
    deadzonePx: 50,
    noisePx: 50,
    mistakeChance: 0.30,
    offscreenTrackingBias: 0.05,
  },
  medium: {
    decisionIntervalMinMs: 150,
    decisionIntervalMaxMs: 270,
    deadzonePx: 35,
    noisePx: 35,
    mistakeChance: 0.20,
    offscreenTrackingBias: 0.10,
  },
  hard: {
    decisionIntervalMinMs: 100,
    decisionIntervalMaxMs: 200,
    deadzonePx: 20,
    noisePx: 18,
    mistakeChance: 0.12,
    offscreenTrackingBias: 0.20,
  },
};
...
class LegacyGameRuntime {
  ...
  _resetBall(toRight = true) {
    ...
    // Randomise the angle
    if (Math.random() > 0.5) this.gameState.ball.dy = 3;
    else this.gameState.ball.dy = -3;
  }
  ...
  _spawnPowerUp() {
    const now = Date.now();
    if (
      now - this.lastSpawnTime > POWERUP_SPAWN_INTERVAL_MS &&
      this.gameState.powerUps.length < 2
    ) {
  ...
  _updateEffect() {
    if (
      this.gameState.activeEffect &&
      Date.now() > this.gameState.activeEffect.expiresAt
    ) {
      this._clearEffect();
    }
  }
  ...
  _checkWinCondition() {
    ...
    if (
      this.gameState.score.p1 >= WIN_SCORE ||
      this.gameState.score.p2 >= WIN_SCORE
    ) {
      this._stopGame();
    }
  }
