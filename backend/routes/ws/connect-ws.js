      ...
      if (fastify.gameStates) {
        for (const [matchId, gameState] of fastify.gameStates.entries()) {
          ...
          if ((leftId === uId || rightId === uId) && !gameState.gameOver) {
            ...
            if (!wasDisconnected) {
              console.log(
                `[WS Connect] User ${userId} connected to match ${matchId} (not a reconnection - not in disconnectedPlayers set)`,
              );
              continue; // Skip reconnection logic for this player
            }

            console.log(
              `[WS Connect] User ${userId} RECONNECTED to match ${matchId}. Was disconnected.`,
            );
            ...
            if (gameState[timeoutKey]) {
              console.log(
                `[WS Connect] Clearing disconnect timeout for ${reconnectedPlayer} in match ${matchId}`,
              );
            ...
            console.log(
              `[WS Connect] Remaining disconnected players: ${[...gameState.disconnectedPlayers].join(", ") || "none"}`,
            );
                    ...
                    disconnectCountdown:
                      gameState.disconnectedPlayers?.size > 0
                        ? {
                            ...
                            gracePeriodEndsAt: gameState.pausedAt
